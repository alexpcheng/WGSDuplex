...
get_sscs_featuremap<-function(input){
  filename = normalizePath(input)
  print("A")
  sample_id = strsplit(filename, '/')[[1]][11]
  print("B")
  df <- fread(filename, sep = '\t', quote = "")
  print("C")
  print(df)
  df <- df[!duplicated(df$MI_pos), ]
  df <- df[df$sscs_bp !='None', ]
  df <- df[df$alt == df$sscs_bp, ]
  ...
  df$cE <- as.numeric(df$cE)
  df$cU <- as.numeric(df$cU)
  df$dS <- as.numeric(df$dS)
  df$FM_passed <- as.numeric(df$FM_passed)
  df$num_N <- as.numeric(df$num_N)
  df$sscs_length <- as.numeric(df$sscs_length)
  df$sscs_PIR <- as.numeric(df$sscs_PIR)
  df$sscs_PIR2 <- as.numeric(df$sscs_PIR2)
  ...
  df = df[(df$alt_bot>=1 & (df$X_FLAGS==16 | df$X_FLAGS==1040)) | (df$alt_top >=1  & (df$X_FLAGS == 0 | df$X_FLAGS == 1024)), ]
  df = df[df$FM_passed==df$cU, ]
  df = df[df$X_EDIST <=2, ]
  df = df[df$cD>=2, ]
  df = df[df$num_N<=5, ]
  df = df[df$sscs_PIR!=-1, ]
  df = df[df$sscs_PIR!="None", ]
  df = df[df$sscs_PIR>=25 & df$sscs_PIR2>=25, ]
